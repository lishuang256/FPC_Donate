...
RESPONSE_HASH=$(jq -r '.[0].responseHash' "$JSON_FILE")
RESPONSE_ENC=$(jq -r '.[0].responseEnc' "$JSON_FILE")
COMMENT_X=$(jq -r '.[0].commentX' "$JSON_FILE")
COMMENT_Y=$(jq -r '.[0].commentY' "$JSON_FILE")
COMMENT_X_NEW=$(jq -r '.[0].commentXNew' "$JSON_FILE")
COMMENT_Y_NEW=$(jq -r '.[0].commentYNew' "$JSON_FILE")
DEPOSITE=$(jq -r '.[0].deposite' "$JSON_FILE")
FEEDBACK_HASH=$(jq -r '.[0].feedbackHash' "$JSON_FILE")
FEEDBACK_ENC=$(jq -r '.[0].feedbackEnc' "$JSON_FILE")
TIMESTAMP=$(jq -r '.[0].timestamp' "$JSON_FILE")
...
donate_docc_test() {
    ...
    try_out_r ${PEER_CMD} chaincode invoke -o ${ORDERER_ADDR} -C ${CHAN_ID} -n ${CC_ID} -c '{"Function":"Donate", "Args": ["'"${RESPONSE_HASH}"'", "'"${DEPOSITE}"'", "'"${RESPONSE_ENC}"'", "'"${COMMENT_X}"'", "'"${COMMENT_Y}"'", "'"${TIMESTAMP}"'"]}' --waitForEvent
    check_result "OK"
    becho ">>>> Create and sumbit an Deliver1 transaction. Response should be OK"
    try_out_r ${PEER_CMD} chaincode invoke -o ${ORDERER_ADDR} -C ${CHAN_ID} -n ${CC_ID} -c '{"Function":"Deliver1", "Args": ["'"${RESPONSE_HASH}"'", "'"${DEPOSITE}"'", "'"${TIMESTAMP}"'"]}' --waitForEvent
    check_result "OK"
    becho ">>>> Create and sumbit an Store transaction. Response should be OK"
    try_out_r ${PEER_CMD} chaincode invoke -o ${ORDERER_ADDR} -C ${CHAN_ID} -n ${CC_ID} -c '{"Function":"Store", "Args": ["'"${RESPONSE_HASH}"'", "'"${TIMESTAMP}"'"]}' --waitForEvent
    check_result "OK"
    becho ">>>> Create and sumbit an Ship transaction. Response should be OK"
    try_out_r ${PEER_CMD} chaincode invoke -o ${ORDERER_ADDR} -C ${CHAN_ID} -n ${CC_ID} -c '{"Function":"Ship", "Args": ["'"${RESPONSE_HASH}"'", "'"${DEPOSITE}"'", "'"${TIMESTAMP}"'"]}' --waitForEvent
    check_result "OK"
    becho ">>>> Create and sumbit an Distribute transaction. Response should be OK"
    try_out_r ${PEER_CMD} chaincode invoke -o ${ORDERER_ADDR} -C ${CHAN_ID} -n ${CC_ID} -c '{"Function":"Distribute", "Args": ["'"${RESPONSE_HASH}"'", "'"${COMMENT_X_NEW}"'", "'"${COMMENT_Y_NEW}"'", "'"${TIMESTAMP}"'"]}' --waitForEvent
    check_result "OK"
    becho ">>>> Create and sumbit an Deliver2 transaction. Response should be OK"
    try_out_r ${PEER_CMD} chaincode invoke -o ${ORDERER_ADDR} -C ${CHAN_ID} -n ${CC_ID} -c '{"Function":"Deliver2", "Args": ["'"${RESPONSE_HASH}"'", "'"${DEPOSITE}"'", "'"${TIMESTAMP}"'"]}' --waitForEvent
    check_result "OK"
    becho ">>>> Create and sumbit an Receive transaction. Response should be OK"
    try_out_r ${PEER_CMD} chaincode invoke -o ${ORDERER_ADDR} -C ${CHAN_ID} -n ${CC_ID} -c '{"Function":"Receive", "Args": ["'"${RESPONSE_HASH}"'", "'"${FEEDBACK_HASH}"'", "'"${FEEDBACK_ENC}"'", "'"${TIMESTAMP}"'"]}' --waitForEvent
    check_result "OK"
    becho ">>>> Create and sumbit an Refund transaction. Response should be OK"
    try_out_r ${PEER_CMD} chaincode invoke -o ${ORDERER_ADDR} -C ${CHAN_ID} -n ${CC_ID} -c '{"Function":"Refund", "Args": ["'"${RESPONSE_HASH}"'", "1", "1", "1", "'"${TIMESTAMP}"'"]}' --waitForEvent
    check_result "OK"
}
